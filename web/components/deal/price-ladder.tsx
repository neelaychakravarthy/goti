...
export function PriceLadderView({ ladder }: PriceLadderProps) {
  ...
  // Only show the competing-seller row when there's a real BATNA — a
  // sibling job in the same hunt with a target price. Hiding it when 0
  // avoids confusing fake "$0" rows on solo negotiations.
  if (ladder.competing_seller > 0) {
    rows.push({
      label: "Competing seller",
      amount: ladder.competing_seller,
      ring: "green",
    });
  }
