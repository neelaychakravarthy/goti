...
async def test_mock_returns_three_fb_listings(use_mocks):
    ...
    for r in results:
        ...
        # Stream B's Listing uses `id` (not Stream C's earlier `listing_id`).
        assert r.id.startswith("fb_mock_")
        assert "standing desk" in (r.title or "").lower()
        # Stream B's Listing uses dollar floats (not Stream C's earlier
        # integer cents). Fixture mins are well above $0.
