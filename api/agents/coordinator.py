...
Shares `app` (and therefore `app.memory`) with the other reasoners —
see `_af_app.py`. Invoked from FastAPI when the user clicks
...
Actual job-row creation + reasoner dispatch happens in FastAPI which
consumes this reasoner's return value.

`app.memory.get` / `app.memory.set` are async against agentfield
0.1.84. See `_af_app.py` docstring for the full shared-memory contract.
"""
...
async def spawn_negotiations(
    goal_id: str,
    listings: list[dict],  # noqa: ARG001 — accepted for context; orchestration may use it
    ...
    Returns ``{job_ids, status, batna_seed}``. FastAPI consumes this to
    create Job rows + dispatch negotiator reasoners.
    """
