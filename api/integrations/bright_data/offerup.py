"""Bright Data raw -> `Listing` parser for OfferUp.
...
def parse_offerup_listings(rows: Iterable[dict]) -> list[Listing]:
    out: list[Listing] = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        listing_id = first(row, "listing_id", "id", "item_id", "offer_id")
        ...
        out.append(
            Listing(
                ...
                title=first(row, "title", "name", "item_title"),
                description=first(row, "description", "body", "details"),
                price_cents=coerce_price_cents(first(row, "price", "price_usd", "amount")),
                currency=str(first(row, "currency", "price_currency") or "USD"),
                url=first(row, "url", "link", "permalink", "item_url"),
                raw=row,
            )
        )
    return out
