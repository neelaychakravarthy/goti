"""End-to-end mock-side demo of the Goti negotiation flow.

Walks Stream C's discovery + Actionbook drivers in the same order Stream B's
agent loop will eventually exercise them, so you can SEE the data shape
end-to-end today without any LLM or real marketplace calls.

Run (from the repo root):
    GOTI_USE_MOCKS=1 ./api/.venv/bin/python scripts/demo_negotiation.py

Note: this is a dev-only CLI helper. Emoji section dividers are intentional
(visual scannability when run live); the project's no-emoji rule is for
production code and docs, not this script.
"""
...
async def main() -> None:
    ...
    for mp in MARKETPLACES:
        ...
        for l in mp_listings[:DISPLAY_PER_MARKETPLACE]:
            print(f"    - {l.listing_id}  {_fmt_price(l.price_cents)}  {l.title}")
    ...
    print(f"\n  fb       -> {fb_pick.listing_id}  {_fmt_price(fb_pick.price_cents)}  {fb_pick.title}")
    print(f"  nextdoor -> {nd_pick.listing_id}  {_fmt_price(nd_pick.price_cents)}  {nd_pick.title}")
    ...
    fb_msg_id = await fb.send_message(PROFILE_ID, fb_pick.listing_id, fb_opening)
    nd_msg_id = await nextdoor.send_message(PROFILE_ID, nd_pick.listing_id, nd_opening)
    ...
    fb_replies = await fb.fetch_replies(PROFILE_ID, fb_pick.listing_id, since_ts)
    nd_replies = await nextdoor.fetch_replies(PROFILE_ID, nd_pick.listing_id, since_ts)
    ...
    print()
    _divider("=")
    print("🧠  BATNA HINT  (sketch — what Stream B's agent would do next)")
    _divider("=")
    print(
